...
struct dir
{
    std::vector<std::variant<file, dir>> nodes;
};

inline void process_dir(std::istringstream & in, dir & cwd)
{
    std::string line;
...
using dir_sizes_t = std::multiset<int, std::greater<>>;

inline int compute_sizes(dir const & cwd, dir_sizes_t & dir_sizes)
{
    int total = 0;
...
inline constexpr auto test_terminal =
R"(
$ cd /
$ ls
dir a
...
inline constexpr auto terminal =
R"(
$ cd /
$ ls
dir fchrtcbh
